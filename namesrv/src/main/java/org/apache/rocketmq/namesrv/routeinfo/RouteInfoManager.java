...
public class RouteInfoManager {
    ...
    // ，然后 Name Server 每隔 10s 扫描 brokerLiveTable ，如 果连续120s 没有收到心跳包， NameServer 将移除该 Broker 的路由信息 同时关闭 Socket 连接
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 判断broker所属集群是否存在,如果不存在则创建,然后将broker名加入到集群broker集合中
                ...
                //维护 BrokerData 信 息 ，
                // 首 先从 brokerAddrTable 根据 BrokerName 尝试获取 Broker 信息 ，
                // 如 果 不存在， 则 新建 BrokerData 并放入 到 brokerAddrTable , registerFirst 设 置为 true ；
                // 如果存在 ， 直接替换原先的， registerFirst 设置为 false，表示非第一次注册 。
                ...
                //如果 Broker 为 Master ，并且 Broker Topic 配置信息发生变化或者是初次注册 ， 则需要创建或更新 Topic 路 由元数据，填充 topicQueueTable ，
                ...
                // 注册 Broker 的过滤器 Server 地址列表 ，一个 Broker 上会关联多个 FilterServer 消息过滤服务器，
    ...
    /**
     * 遍历 brokerLivelnfo 路由表（HashMap ），
     * 检测 BrokerLiveInfo 的 lastUpdateTimestamp 上次收到心跳包的时间如果超过当前时间120s, NameServer 则认为该 Broker 已不可用，
     * 故需要将它移除，关闭 Channel ，然后删除与该 Broker 相关的路由信息，
     */
    ...
    public void onChannelDestroy(String remoteAddr, Channel channel) {
        ...
        if (brokerAddrFound != null && brokerAddrFound.length() > 0) {
            ...
            try {
                try {
                    // Step:1
                    // 申请写锁根据brokerAddress从brokerLiveTable/filterServerTable移除
                    this.lock.writeLock().lockInterruptibly();
                    this.brokerLiveTable.remove(brokerAddrFound);
                    this.filterServerTable.remove(brokerAddrFound);

                    // Step:2
                    // 从brokerAddrTable 的 brokerData中找到具体的broker,从brokerData中移除
                    // 如果此时brokerData内不含有其他broker,则在从brokerAddrTable 移除该条记录
                    ...
                    // Step:3
                    // 根据BrokerName从clusterAddrTable中找到Broker并从集群中移除
                    // 移除后如果集群不包含任何Broker,则将该集群从clusterAddrTable中删除
                    ...
                    // Step:4
                    // 根据brokerName遍历所有主题的队列,如果队列中有当前Broker的队列,则移除该队列.
                    // 移除后如果Topic不包含任何队列,则将该Topic移除
                    if (removeBrokerName) {
